			...
			if user.password==request.POST['password']:
				if user.firstlogin==False:
					...
					if user.usertype=="buyer":
						...
						carts=Cart.objects.filter(user=user,paytm_status=False)
						request.session['cart_count']=len(carts)
						return render(request,'index.html')
					else:
						request.session['email']=user.email
				...
				msg='Password & Confirm Password is Incorrect'
...
def logout(request):
	try:
		...
		del session.session['profile_pic']
		return render(request,'login.html')
...
def verify_otp(request):
	...
	if otp==uotp:
		...
		msg="Invalid OTP"
		return render(request,'new-password.html',{'email':email,'otp':otp,'msg':msg})
...
def seller_edit_product(request,pk):
	...
	if request.method=="POST":
		...
		try:
			product.profile_pic=request.FILES['product_pic']
		except:
			pass
		product.save()
